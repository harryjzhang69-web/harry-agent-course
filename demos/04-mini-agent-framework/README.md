...
对应 [Part 2 第08 章](../../docs/part2/08-从零写一个Agent框架.md)。一个不到300 行、从零手写的最小 Agent 框架，覆盖工具注册、ReAct 循环、基础记忆三个核心能力。目的不是"造一个能用的产品级框架"，而是让你看清楚 LangGraph/AutoGen 这类重型框架底层到底在解决什么问题。
...
- `Agent.run()` 目前是单Agent 循环，如果要支持多Agent协作，需要在这基础上加一层"消息路由"（这一步复杂度上升明显，值不值得做，参考 Part 2 第07章的框架选型讨论）
