...
**Q1.** 判断一个系统是不是 Agent，最关键看什么？
A. 是否用了大模型 API　B. 决策权是否掌握在模型手里，能否自主判断分支　C. 是否有多个 Agent 协作　D. 代码行数是否够多

**Q2.** Dify/Coze 这类低代码平台搭出来的工作流，在连续谱上属于哪个位置？
A. 位置1 固定脚本　B. 位置2 条件流程编排　C. 位置4 自主规划循环　D. 位置5 多智能体协作

**Q3.** 下面哪一项**不是**本章提到的常见误区？
A. 调了 LLM API 就是 Agent　B. 工作流编排=Agent　C. 必须多智能体协作才叫高级 Agent　D. 用了向量数据库才是 Agent

**Q4.** 判断需要用 L3 多智能体协作的关键条件是什么？
A. 任务比较复杂　B. 任务天然需要多个专业角色分工，且单一角色确实扛不住　C. 老板要求　D. 显得技术含量高

**Q5.** Agent 的核心闭环是什么？
A. 输入-输出　B. 感知-决策-行动　C. 训练-推理-部署　D. 计划-执行-复盘
