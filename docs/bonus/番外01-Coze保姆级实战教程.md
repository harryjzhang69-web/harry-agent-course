...
## 常见踩坑（建议你实操一遍后，改写成自己的真实踩坑记录）

- **知识库检索不准**：分段太长导致一段里混了多道题，模型检索时容易"张冠李戴"——按上面 Step3 的分段建议调整。
- **Bot 记不住上一轮问的是哪道题**：默认的多轮对话依赖模型自己维护上下文，如果发现"追问"和"上一题"脱节，可以在 Prompt 里显式要求"在追问时重复一遍你上一次问的是哪道题"。
- **发布到微信/抖音审核卡住**：这类渠道涉及资质审核，周期不可控，练手阶段建议先用分享链接/API，不要一开始就卡在审核流程上。
- **额度用超**：个人版有免费调用额度上限，知识库调用+大模型调用都算，测试阶段建议控制测试轮次，别一次性跑几十轮调试。
